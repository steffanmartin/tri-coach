...
way to correct a bad day.
    ...
    python -m coach.wellness_sync                       # last 7 days (the cron path)
"""
import argparse
import asyncio
import json
...
def sync(days: int, dry_run: bool = False) -> list[dict]:
    rows = google_health.wellness_rows(days)
    if dry_run:
        return rows
    push(rows)
    return rows
...
async def main(days: int = 7) -> None:
    """Scheduled entry point. Failures are reported, never swallowed — a silent
    failure here shows up as a missing HRV reading the coach grades around."""
    try:
        # Off-thread: these are blocking HTTP calls and the bot's event loop is
        # busy long-polling Telegram.
        rows = await asyncio.to_thread(sync, days)
...
def _cli() -> None:
    ...
    rows = sync(args.days, dry_run=args.dry_run)
